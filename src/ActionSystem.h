...
private:
    ...
    void combatOutcome(int const tick, std::shared_ptr<Entity> &attacker, std::shared_ptr<Entity> &defender, std::vector<EntityPos> &pendingDrops, Statistics &statistics);

public:
    ActionSystem(DecisionSystem &decision,
                 MovementSystem &movement,
                 Grid &grid);
    void update(int tick, EntityManager &em, std::mt19937 &rng, std::vector<EntityPos> &pendingDrops, Statistics &statistics);
};
